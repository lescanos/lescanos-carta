...
async function saveEdit() {
  ...
  const { error } = await supabase.from('perfiles').update({
    ...
    login_key: editKey.value.trim().toLowerCase(),
    rol:       editRol.value,
...
function openPwd(p: Perfil) {
  pwdUserId.value  = p.id
  pwdNombre.value  = p.nombre
  pwdValue.value   = ''
...
async function savePwd() {
  ...
  showToast(`Contraseña de ${pwdNombre.value} actualizada ✓`)
}
...
async function createUser() {
  ...
  const { ok, error } = await callAdminFn({
    ...
    data: {
      ...
      login_key: newKey.value.trim().toLowerCase(),
      rol:       newRol.value,
  ...
  showToast(`${newNombre.value} creado ✓`)
}
...
async function saveCostoEnvio() {
  ...
  const { error } = await supabase.from('config')
    .upsert({ clave: 'costo_envio', valor: String(val) }, { onConflict: 'clave' })
...
<template>
  <div class="fixed inset-0 flex flex-col bg-dark text-white overflow-hidden">
    ...
    <header class="bg-dark2 border-b border-gold/[.18] px-4 py-3 flex items-center gap-2 flex-shrink-0 min-h-[52px]">
      ...
      <span class="font-display text-[1rem] font-black text-gold tracking-[.06em] flex-1">Gestión de usuarios</span>
    ...
    <div class="flex-1 overflow-y-auto">
      <div v-if="loading" class="p-10 text-center text-gray-500 text-[.85rem]">Cargando...</div>
        ...
        <div v-for="p in perfiles" :key="p.id"
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2">
              ...
              <span v-if="!p.activo"
                class="text-[.55rem] text-red-400/60 border border-red-400/20 px-1.5 py-0.5 rounded tracking-[.08em] uppercase">
                Inactivo
              </span>
            </div>
          ...
          <div class="flex gap-1.5 flex-shrink-0">
            ...
            <button @click="openPwd(p)"
              class="text-[.65rem] px-2 py-1 rounded border border-gold/20 text-gold/60 bg-transparent cursor-pointer tracking-[.03em]">
              🔑
            </button>
            ...
            <button @click="deleteUser(p)"
              class="text-[.65rem] px-2 py-1 rounded border border-red-500/20 text-red-400/50 bg-transparent cursor-pointer">
              ✕
            </button>
          </div>
        </div>
      ...
      <!-- Config section -->
      <div class="px-4 pt-5 pb-4 border-t border-white/[.05] mt-4">
    ...
    <Transition name="sheet">
      <div v-if="editVisible" class="fixed inset-0 bg-black/72 z-50 flex items-end" @click.self="editVisible = false">
        <div class="w-full bg-[#1a1a1a] rounded-t-[22px] border-t border-gold/30 pt-2 pb-10 max-h-[92vh] overflow-y-auto">
          ...
          <div class="font-display text-[1rem] font-black text-gold text-center tracking-[.06em] pb-3 border-b border-white/[.06]">
            Editar perfil
          </div>
          ...
          <div class="flex flex-wrap gap-2 justify-center px-5 pt-4 pb-2">
            <button v-for="e in EMOJIS" :key="e" @click="editEmoji = e"
              :class="['text-xl w-10 h-10 rounded-xl border-2 flex items-center justify-center cursor-pointer transition-colors',
                editEmoji === e ? 'border-gold bg-gold/[.1]' : 'border-gold/10 bg-transparent']">
              {{ e }}
            </button>
          </div>
          ...
          <button @click="editVisible = false"
            class="block w-[calc(100%-32px)] mx-4 mt-2 py-2.5 text-white/35 border border-white/10 rounded-lg text-[.82rem] bg-transparent cursor-pointer">
            Cancelar
          </button>
        </div>
      </div>
        ...
        <div class="w-full bg-[#1a1a1a] rounded-t-[22px] border-t border-gold/30 pt-2 pb-10 max-h-[92vh] overflow-y-auto">
          ...
          <div class="flex flex-wrap gap-2 justify-center px-5 pt-4 pb-2">
            <button v-for="e in EMOJIS" :key="e" @click="newEmoji = e"
              :class="['text-xl w-10 h-10 rounded-xl border-2 flex items-center justify-center cursor-pointer transition-colors',
                newEmoji === e ? 'border-gold bg-gold/[.1]' : 'border-gold/10 bg-transparent']">
              {{ e }}
            </button>
          </div>
